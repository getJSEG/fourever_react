...
const LineItem = ({item , deleteSelf, skuItem, updateQty }) => {
    ...
    return (
        <li className= {`pos-line-item ${ openMoreInfo ? 'pos-line-item-options-open': ''}`}> 
            <div> 
                ...
                <span  className="pos-qty-input">{item?.units}</span>
                ...
                <span className="pos-qty-total">{  qty >= 0 || qty === '' ?  formatCurrecy(item?.units * item?.unitPrice) : formatCurrecy(0) }</span>
            ...
            
            {
                ...
                (<span className="shoppingcart-change-qty"> 
                    <button  className="shoppingcart-reduce-qty-button" onClick={ (e) => updateQty(e, item?.sku, 'diff') } > - </button>
                    <input 
                        value={qty}
                        onInput={ (e) => updateQty(e, skuItem, 'manual') }
                    />
                    <button className="shoppingcart-add-qty-button" onClick={ (e) => updateQty(e, skuItem, 'add') } > + </button>
                </span>) : null
            }
